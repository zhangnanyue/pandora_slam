#include "lidar_visual_alignment/Calibrating.h"
#include <filesystem>

// string strSettingPath =
// "/home/zyw/catkin_ws/src/camvox/isee-camvox/camvox/config/camera.yaml";
// string RGBPath =
// "/home/zyw/catkin_ws/src/camvox/isee-camvox/camvox/calibration/calibration.bmp";
// string PcdPath =
// "/home/zyw/catkin_ws/src/camvox/isee-camvox/camvox/calibration/calibration.pcd";
...
std::string root_path = "/home/caros/pandora_slam/";
std::string project_name = "lidar_visual_alignment";
std::filesystem::path config_path =
    std::filesystem::path(root_path) / "config" / project_name / "camera.yaml";
std::filesystem::path img_path =
    std::filesystem::path(root_path) / "data" / project_name / "alignment.bmp";
std::filesystem::path pcd_path =
    std::filesystem::path(root_path) / "data" / project_name / "alignment.pcd";
string strSettingPath_ = config_path.string();
string RGBPath_ = img_path.string();
string PcdPath_ = pcd_path.string();

// kdtree and pcd setting
pcl::KdTreeFLANN<pcl::PointXYZ> kdtree_;
pcl::PointCloud<pcl::PointXYZI>::Ptr
    horizon_cloud_(new pcl::PointCloud<pcl::PointXYZI>);
pcl::PointCloud<pcl::PointXYZ>::Ptr
    rgb_cloud_xyz_(new pcl::PointCloud<pcl::PointXYZ>),
    lidar_cloud_xyz_(new pcl::PointCloud<pcl::PointXYZ>),
    rgb_use_cloud_(new pcl::PointCloud<pcl::PointXYZ>),
    lidar_use_cloud_(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZRGB>::Ptr
    rgb_cloud_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGB>),
    lidar_cloud_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGB>),
    rgb_use_cloud_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGB>),
    lidar_use_cloud_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGB>);

/********************************************Calibration**************************************************************************/
Calibrating::Calibrating(string _strSettingPath, string _RGB_path,
                         string _Pcd_path, string _Projection_type,
                         bool _isEnhanceImg, bool _isFillImg)
    : mbResetRequested(false), mbFinishRequested(false), mbFinished(true),
      mbStopped(false), mbStopRequested(false), mbNotStop(false),
      mbCalibrating(false), mbOptimizing(false) {
  ...
  Eigen::Matrix3d test_rotation_matrix =
      rotation_matrix * rotation_matrix_adjust;
  cout << "use rotation matrix =\n" << test_rotation_matrix << endl;
  Eigen::Vector3d test_angle = test_rotation_matrix.eulerAngles(2, 1, 0);
  extrinsic_params << test_angle[0], test_angle[1], test_angle[2], T_[0], T_[1],
      T_[2];

  // read Camera image
  origin_RGB_ = imread(_RGB_path, cv::IMREAD_UNCHANGED);
  imRGB_ = imread(_RGB_path, cv::IMREAD_UNCHANGED);
  ...
  // RGB2GRAY
  cvtColor(imRGB_, imGray_, cv::COLOR_RGB2GRAY);
  cv::imshow("rect image gray", imGray_);
  cv::imwrite("/home/caros/pandora_slam/debug/lidar_visual_alignment/"
              "rect_image_gray.png",
              imGray_);
  cv::waitKey();
  ...
  EdgeDetction(projection_type_, rgb_canny_threshold_, depth_canny_threshold_,
               intensity_canny_threshold_);

  // // Edge filter and build Point cloud
  BuildRgbCloud();
  BuildLidarCloud();
  std::cout << "rgb_cloud_xyz_:" << rgb_cloud_xyz_->points.size() << std::endl;
  std::cout << "lidar_cloud_xyz_:" << lidar_cloud_xyz_->points.size()
            << std::endl;
  pcl::io::savePCDFileASCII(
      "/home/caros/pandora_slam/debug/lidar_visual_alignment/rgb_cloud.pcd",
      *rgb_cloud_xyz_);
  std::cout << "Saved " << rgb_cloud_xyz_->points.size()
            << " data points to test_pcd.pcd." << std::endl;
  pcl::io::savePCDFileASCII(
      "/home/caros/pandora_slam/debug/lidar_visual_alignment/lidar_cloud.pcd",
      *lidar_cloud_xyz_);
  std::cout << "Saved " << lidar_cloud_xyz_->points.size()
            << " data points to test_pcd.pcd." << std::endl;

  // initRgbUseClouds();

  // rgb_use_cloud_xyzrgb_ = pcl::PointCloud<pcl::PointXYZRGB>::Ptr(
  //     new pcl::PointCloud<pcl::PointXYZRGB>);
  // lidar_use_cloud_xyzrgb_ = pcl::PointCloud<pcl::PointXYZRGB>::Ptr(
  //     new pcl::PointCloud<pcl::PointXYZRGB>);
  // convertToRgbCloud(rgb_cloud_xyz_, rgb_cloud_xyzrgb_,
  //                   Eigen::Vector3i(255, 0, 0));
  // convertToRgbCloud(lidar_cloud_xyz_, lidar_cloud_xyzrgb_,
  //                   Eigen::Vector3i(0, 0, 255));
  // std::cout << "rgb_cloud_xyz_:" << rgb_cloud_xyz_->points.size() <<
  // std::endl; float init_distance = calcMeanDistance(rgb_use_cloud_,
  // lidar_use_cloud_); cout << "init distance:" << init_distance << endl;
  // convertToRgbCloud(rgb_use_cloud_, rgb_use_cloud_xyzrgb_,
  //                   Eigen::Vector3i(255, 0, 0));
  // convertToRgbCloud(lidar_use_cloud_, lidar_use_cloud_xyzrgb_,
  //                   Eigen::Vector3i(0, 0, 255));
}
...
bool Calibrating::loadPcd(const std::string &pcd_file) {
  ...
  if (pcl::io::loadPCDFile(pcd_file, *horizon_cloud_) != -1) {
    std::cout << "load pcd file successfully! size:"
              << horizon_cloud_->points.size() << std::endl;
    return true;
  } else {
...
//***********************************************Projection***********************************************************************/
void Calibrating::Projection(const ProjectionType projection_type,
  ...
  if (projection_type == INTENSITY) {
    ...
    imIntensity_ = imIntensity_(rect_);
    imIntensity_.convertTo(imIntensity8_, CV_8U, 1.0 / 256);
    // cv::imshow("project by intensity 8u", imIntensity_);
    // cv::imwrite("/home/caros/pandora_slam/debug/lidar_visual_alignment/"
    //             "intensity_8u.png",
    //             imIntensity_);
    // cv::waitKey();
    ...
    imdepth_ = imdepth_(rect_);
    imdepth_.convertTo(imdepth8_, CV_8U, 1.0 / 256);
    // cv::imshow("project by depth 8u", imdepth8_);
    // cv::imwrite(
    //     "/home/caros/pandora_slam/debug/lidar_visual_alignment/depth_8u.png",
    //     imdepth8_);
    // cv::waitKey();
    if (is_enhancement_)
      equalizeHist(imdepth8_, imdepth8_);
...
//*****************************************************convertToRgbCloud*****************************************************************/
void Calibrating::convertToRgbCloud(
    pcl::PointCloud<pcl::PointXYZ>::Ptr &xyz_cloud,
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr &xyzrgb_cloud, Eigen::Vector3i rgb) {
...
Mat Calibrating::convertByIntensity(const Vector6d &calib_params) {
  ...
  cv::Mat t_vec = (cv::Mat_<double>(3, 1) << calib_params[3], calib_params[4],
                   calib_params[5]);
        ...
        for (int x_inc = 0; x_inc < 4; x_inc++) {
          for (int y_inc = 0; y_inc < 4; y_inc++) {
            ...
            if (x_near > 0 && y_near > 0 &&
              ...
              temp_intensity.push_back(
                  image_project.at<ushort>(y_near, x_near));
            }
          }
  ...
  cv::imwrite(
      "/home/caros/pandora_slam/debug/lidar_visual_alignment/intensity.png",
      filed_img);
  cv::waitKey();
  /************************Debug********************************/
  return filed_img;
...
//*************************************************convertByDepth*********************************************************************/
Mat Calibrating::convertByDepth(const Vector6d &calib_params,
                                const bool is_fillImg) {
  std::vector<cv::Point3f> pts_3d;
  Eigen::AngleAxisd rotation_vector3;
  rotation_vector3 =
      Eigen::AngleAxisd(calib_params[0], Eigen::Vector3d::UnitZ()) *
  ...
  for (size_t i = 0; i < pts_2d.size(); ++i) {
    ...
    if (point_2d.x <= 0 || point_2d.x >= image_cols || point_2d.y <= 0 ||
      ...
      float depth = sqrt(pow(horizon_cloud_->points[i].x, 2) +
                         pow(horizon_cloud_->points[i].y, 2) +
                         pow(horizon_cloud_->points[i].z, 2));
      if (depth > max_depth) {
        depth = 65535;
      } else {
  ...
  cv::imwrite("/home/caros/pandora_slam/debug/lidar_visual_alignment/depth.png",
              image_project);
  cv::waitKey();
  /************************Debug********************************/
  return filed_img;
...
//*************************************************EdgeDetction*********************************************************************/
void Calibrating::EdgeDetction(const ProjectionType projection_type,
                               const int rgb_threshold,
  ...
  findContours(rgb_canny_, contours_RGB_, hierarchy_RGB_, cv::RETR_EXTERNAL,
               cv::CHAIN_APPROX_NONE, Point());

  if (projection_type == INTENSITY) {
    blur(imIntensity8_, im_intensity, Size(5, 5), Point(-1, -1));
    Canny(im_intensity, intensity_canny_, intensity_threshold,
          intensity_threshold * 3, 3, true);
    findContours(intensity_canny_, contours_intensity_, hierarchy_intensity_,
                 cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE, Point());
  } else if (projection_type == DEPTH) {
    GaussianBlur(imdepth8_, im_depth, cv::Size(gaussian_size, gaussian_size), 0,
                 0);
    Canny(im_depth, depth_canny_, depth_threshold, depth_threshold * 3, 3,
          true);
    findContours(depth_canny_, contours_imdepth_, hierarchy_imdepth_,
                 cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE, Point());
  } else {
    blur(imIntensity8_, im_intensity, Size(5, 5), Point(-1, -1));
    Canny(im_intensity, intensity_canny_, intensity_threshold,
    ...
    findContours(intensity_canny_, contours_intensity_, hierarchy_intensity_,
                 cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE, Point());
    cv::imshow("canny by intensity", intensity_canny_);
    cv::imwrite(
        "/home/caros/pandora_slam/debug/lidar_visual_alignment/intensity_canny.png",
        intensity_canny_);
    cv::waitKey();
    GaussianBlur(imdepth8_, im_depth, cv::Size(gaussian_size, gaussian_size), 0,
                 0);
    ...
    findContours(depth_canny_, contours_imdepth_, hierarchy_imdepth_,
                 cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE, Point());
    cv::imshow("canny by depth", depth_canny_);
    cv::imwrite(
        "/home/caros/pandora_slam/debug/lidar_visual_alignment/depth_canny.png",
        depth_canny_);
    cv::waitKey();
  }
}
...
//***************************************************extractEdge*******************************************************************/
void Calibrating::extractEdge(const bool is_filter, const Mat &img,
                              const vector<vector<Point>> &contous,
                              const int min_len, Mat &edge_img,
  ...
  float radius_min = 5;
  float radius_max = 8;
  pcl::PointCloud<pcl::PointXYZ>::Ptr kd_input_cloud(
    ...
    for (int i = 0; i < edge_img.rows; i++) {
      for (int j = 0; j < edge_img.cols; j++) {
        ...
        if (img.at<uchar>(i, j) == 0) {
          if (kdtree->radiusSearch(searchPoint, radius_min,
                                   pointIdxRadiusSearch,
                                   pointRadiusSquaredDistance) <= 0) {
            if (kdtree->radiusSearch(searchPoint, radius_max,
                                     pointIdxRadiusSearchMax,
                                     pointRadiusSquaredDistanceMax) > 0) {
              for (size_t k = 0; k < pointIdxRadiusSearchMax.size(); k++) {
                edge_img.at<uchar>(
                    kd_input_cloud->points[pointIdxRadiusSearchMax[k]].y,
                    kd_input_cloud->points[pointIdxRadiusSearchMax[k]].x) = 255;
              }
            }
          }
        }
...
void Calibrating::BuildRgbCloud() {
  ...
  rgb_cloud_xyzrgb_ = pcl::PointCloud<pcl::PointXYZRGB>::Ptr(
      new pcl::PointCloud<pcl::PointXYZRGB>);
  extractEdge(is_use_filter, rgb_canny_, contours_RGB_, 50, color_edge_img,
              rgb_cloud_xyz_);
  convertToRgbCloud(rgb_cloud_xyz_, rgb_cloud_xyzrgb_,
...
void Calibrating::BuildLidarCloud() {
  ...
  lidar_cloud_xyz_ =
      pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
  lidar_cloud_xyzrgb_ = pcl::PointCloud<pcl::PointXYZRGB>::Ptr(
      new pcl::PointCloud<pcl::PointXYZRGB>);

  pcl::PointCloud<pcl::PointXYZ>::Ptr intensity_cloud_xyz =
      pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZ>::Ptr depth_cloud_xyz =
      pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
  if (projection_type_ == INTENSITY) {
    Mat intensity_edge_img = intensity_canny_;
    extractEdge(is_use_filter, intensity_canny_, contours_intensity_, 150,
                intensity_edge_img, intensity_cloud_xyz);
  } else if (projection_type_ == DEPTH) {
    Mat depth_edge_img = depth_canny_;
    extractEdge(is_use_filter, depth_canny_, contours_imdepth_,
                depth_edge_threshold_, depth_edge_img, depth_cloud_xyz);
  } else {
    Mat intensity_edge_img = intensity_canny_;
    extractEdge(is_use_filter, intensity_canny_, contours_intensity_, 150,
                intensity_edge_img, intensity_cloud_xyz);
...
float Calibrating::costFunction(const double *abc) {
  ...
  adjust_rotation_matrix =
      Eigen::AngleAxisd(eluer[0], Eigen::Vector3d::UnitZ()) *
      Eigen::AngleAxisd(eluer[1], Eigen::Vector3d::UnitY()) *
      Eigen::AngleAxisd(eluer[2], Eigen::Vector3d::UnitX());
  Eigen::Matrix3d true_rotation_matrix =
      init_rotaion_matrix * adjust_rotation_matrix;
  Eigen::Vector3d true_euler = true_rotation_matrix.eulerAngles(2, 1, 0);
  extrinsic_params << true_euler[0], true_euler[1], true_euler[2], T_[0], T_[1],
      T_[2];
  float distance = 1000;
  Projection(projection_type_, extrinsic_params);
  t2 = clock() - t2;
...
void Calibrating::color2d(const bool use_depth, const string info) {
  ...
  cv::Size text_size =
      cv::getTextSize(info, font_face, font_scale, thickness, &baseline);

  cv::Point origin;
  origin.x = text_size.width / 2;
  origin.y = 15 + text_size.height / 2;
  cv::putText(im_show, info, origin, font_face, font_scale, cv::Scalar(0, 0, 0),
              thickness, 8, 0);
...
/******************************************************color3d****************************************************************/
void Calibrating::color3d(const Vector6d &calib_params, const int density,
  ...
  for (size_t i = 0; i < pts_2d.size(); i = i + density) {
    if (pts_2d[i].x > min_x && pts_2d[i].x < max_x && pts_2d[i].y > min_y &&
        pts_2d[i].y < max_y) {
...
void Calibrating::showClouds(
  ...
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(
      new pcl::visualization::PCLVisualizer("Org Viewer"));
...
void Calibrating::showOrgPointClouds(const string name) {
  ...
  // 原始点云数据显示
  show_cloud->width =
      lidar_cloud_xyzrgb_->points.size() + rgb_cloud_xyzrgb_->points.size();
  show_cloud->height = 1;
...
void Calibrating::showNearestPointClouds(float radius) {
  ...
  show_cloud->height = 1;
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(
      new pcl::visualization::PCLVisualizer("Knearest Viewer"));
  pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(
      show_cloud);
  viewer->addPointCloud(show_cloud, rgb, "sample cloud");
  viewer->setPointCloudRenderingProperties(
...
//****************************************************calcMeanDistance******************************************************************/
float Calibrating::calcMeanDistance(
    pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud1,
    pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud2) {
  // test
  ...
  float un_match =
      (2 * rgb_cloud_xyz_->size() - count) * 1.0 / (2 * rgb_cloud_xyz_->size());
  cout << "ave distance:" << mean_distance / count
       << " un_match:" << un_match * 5
       << " cost: " << (mean_distance / count + un_match * 5) << endl;
  return (mean_distance / count + un_match * 5);
}

/*********************************************************testRegister*************************************************************/
void Calibrating::testRegister(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1,
                               pcl::PointCloud<pcl::PointXYZ>::Ptr cloud2) {
...
bool Calibrating::initRgbUseClouds() {
    ...
    if (tree->nearestKSearch(searchPoint, K, pointIdxNKNSearch,
      ...
      for (int j = 0; j < K; j++) {
        float distance = calcDistance(
            searchPoint, rgb_cloud_xyz_->points[pointIdxNKNSearch[j]]);
  ...
  if (lidar_use_cloud_->points.size() == 0 ||
      rgb_use_cloud_->points.size() == 0) {
    return false;
  }
  return true;
}
...
// update parameter
// 拋棄異常值（超過上線，或者超過下線），根據均值可信範圍，選擇是否接受標定的結果並跟新進配置文件
//!  ---------------------------------------------------  Run
//!  --------------------------------------------------------------------------
//!  //
void Calibrating::Run() {
  ...
  while (1) {
    ...
    if (mbOptimizing) {
      ...
      // read Camera image
      origin_RGB_ = imread(RGBPath_, cv::IMREAD_UNCHANGED);
      imRGB_ = imread(RGBPath_, cv::IMREAD_UNCHANGED);
      ...
      for (int iter_count = 0; iter_count < search_round; iter_count++) {
        for (int round_index = 0; round_index < 3; round_index++) {
          ...
          if (optimize_type == 0) {
            ...
            for (int i = 0; i < single_search_count; i++) {
              float roll =
                  angle[0] + pow(-1, i) * int(i / 2) * single_resolution;
        ...
        cout << "After round " << iter_count << " best rpy:" << rad2deg(best_r_)
             << "," << rad2deg(best_p_) << "," << rad2deg(best_y_) << endl;
        optimize_type = 0;
        double test_angle[1] = {best_r_};
...
void Calibrating::Release() {
  ...
  // for (list<KeyFrame *>::iterator lit = mlNewKeyFrames.begin(), lend =
  // mlNewKeyFrames.end(); lit != lend; lit++)
  //    delete *lit;
...
void Calibrating::InformCalibrating(const bool &flag) { mbCalibrating = flag; }

void Calibrating::InformOptimizing(const bool &flag) { mbOptimizing = flag; }
