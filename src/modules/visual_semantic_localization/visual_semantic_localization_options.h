...
struct VisualSemanticLocalizationOptions {
  ...
  // 手动解析相机内参YAML文件的辅助函数
  static bool LoadCameraIntrinsicsFromYaml(const std::string& yaml_file, 
  ...
  // 从YAML文件加载配置
  static bool LoadFromYaml(const std::string& yaml_file, VisualSemanticLocalizationOptions& options) {
    ...
    // 读取config部分
      ...
      parser->ParseConfig("config.extrinsics_backlidar_imu", options.config.extrinsics_backlidar_imu_file);
      parser->ParseConfig("config.extrinsics_camera_backlidar", options.config.extrinsics_camera_backlidar_file);
      parser->ParseConfig("config.extrinsics_frontlidar_backlidar", options.config.extrinsics_frontlidar_backlidar_file);
    ...
    // 读取IMU参数
      ...
      // IMU 噪声参数
      parser->ParseConfig("imu.sensor_noise_std.acc_noise_std_x", options.imu.sensor_noise_std.acc_noise_std_x);
      parser->ParseConfig("imu.sensor_noise_std.acc_noise_std_y", options.imu.sensor_noise_std.acc_noise_std_y);
      parser->ParseConfig("imu.sensor_noise_std.acc_noise_std_z", options.imu.sensor_noise_std.acc_noise_std_z);
      parser->ParseConfig("imu.sensor_noise_std.gyro_noise_std_x", options.imu.sensor_noise_std.gyro_noise_std_x);
      parser->ParseConfig("imu.sensor_noise_std.gyro_noise_std_y", options.imu.sensor_noise_std.gyro_noise_std_y);
      parser->ParseConfig("imu.sensor_noise_std.gyro_noise_std_z", options.imu.sensor_noise_std.gyro_noise_std_z);
      
      // IMU 偏置随机游走参数
      parser->ParseConfig("imu.bias_random_walk_std.acc_bias_std_x", options.imu.bias_random_walk_std.acc_bias_std_x);
      parser->ParseConfig("imu.bias_random_walk_std.acc_bias_std_y", options.imu.bias_random_walk_std.acc_bias_std_y);
      parser->ParseConfig("imu.bias_random_walk_std.acc_bias_std_z", options.imu.bias_random_walk_std.acc_bias_std_z);
      parser->ParseConfig("imu.bias_random_walk_std.gyro_bias_std_x", options.imu.bias_random_walk_std.gyro_bias_std_x);
      parser->ParseConfig("imu.bias_random_walk_std.gyro_bias_std_y", options.imu.bias_random_walk_std.gyro_bias_std_y);
      parser->ParseConfig("imu.bias_random_walk_std.gyro_bias_std_z", options.imu.bias_random_walk_std.gyro_bias_std_z);
      
      // IMU 初始标准差参数
      parser->ParseConfig("imu.initial_std.pos_init_std", options.imu.initial_std.pos_init_std);
      parser->ParseConfig("imu.initial_std.vel_init_std", options.imu.initial_std.vel_init_std);
      parser->ParseConfig("imu.initial_std.rot_init_std", options.imu.initial_std.rot_init_std);
      parser->ParseConfig("imu.initial_std.gyro_bias_init_std", options.imu.initial_std.gyro_bias_init_std);
      parser->ParseConfig("imu.initial_std.acc_bias_init_std", options.imu.initial_std.acc_bias_init_std);
      parser->ParseConfig("imu.initial_std.gravity_init_std", options.imu.initial_std.gravity_init_std);
    ...
    parser->ParseConfig("data.raw_image_data_folder", relative_path);
      options.data.raw_image_data_folder_path = GetFullPath(options.data.root_path, relative_path);

    parser->ParseConfig("data.semantic_data_folder", relative_path);
      options.data.semantic_data_folder_path = GetFullPath(options.data.root_path, relative_path);

    parser->ParseConfig("data.raw_imu_data_file", relative_path);
      options.data.raw_imu_data_file_path = GetFullPath(options.data.root_path, relative_path);

    parser->ParseConfig("data.ground_truth_data_file", relative_path);
      options.data.ground_truth_data_file_path = GetFullPath(options.data.root_path, relative_path);

    parser->ParseConfig("data.raw_lidar_data_folder", relative_path);
      options.data.raw_lidar_data_folder_path = GetFullPath(options.data.root_path, relative_path);

    parser->ParseConfig("data.semantic_contours_file", relative_path);
      options.data.semantic_contours_file_path = GetFullPath(options.data.root_path, relative_path);

      // 读取相机内参
      std::string camera_intrinsics_path = GetFullPath(options.config.root_path, options.config.camera_intrinsics_file);
      if (!LoadCameraIntrinsicsFromYaml(camera_intrinsics_path, 
                                      options.camera.width, options.camera.height,
                                      options.camera.intrinsics_K, 
      ...
      // 读取外参
      std::string extrinsics_camera_backlidar_path = GetFullPath(options.config.root_path, options.config.extrinsics_camera_backlidar_file);
      if (!LoadExtrinsicsFromYaml(extrinsics_camera_backlidar_path, options.extrinsics.T_backlidar_camera)) {
      ...
      std::string extrinsics_backlidar_imu_path = GetFullPath(options.config.root_path, options.config.extrinsics_backlidar_imu_file);
      if (!LoadExtrinsicsFromYaml(extrinsics_backlidar_imu_path, options.extrinsics.T_imu_backlidar)) {
      ...
      std::string extrinsics_frontlidar_backlidar_path = GetFullPath(options.config.root_path, options.config.extrinsics_frontlidar_backlidar_file);
      if (!LoadExtrinsicsFromYaml(extrinsics_frontlidar_backlidar_path, options.extrinsics.T_backlidar_frontlidar)) {
  ...
  void Print() const {
    ...
    PRINT_INFO("Raw Lidar Data Folder: %s\n", data.raw_lidar_data_folder_path.c_str());
    ...
    PRINT_INFO("  Accelerometer: x=%e, y=%e, z=%e\n", 
             imu.sensor_noise_std.acc_noise_std_x, imu.sensor_noise_std.acc_noise_std_y, imu.sensor_noise_std.acc_noise_std_z);
    PRINT_INFO("  Gyroscope: x=%e, y=%e, z=%e\n", 
             imu.sensor_noise_std.gyro_noise_std_x, imu.sensor_noise_std.gyro_noise_std_y, imu.sensor_noise_std.gyro_noise_std_z);
    PRINT_INFO("IMU Bias Random Walk Standard Deviation:\n");
    PRINT_INFO("  Accelerometer Bias: x=%e, y=%e, z=%e\n", 
             imu.bias_random_walk_std.acc_bias_std_x, imu.bias_random_walk_std.acc_bias_std_y, imu.bias_random_walk_std.acc_bias_std_z);
    PRINT_INFO("  Gyroscope Bias: x=%e, y=%e, z=%e\n", 
             imu.bias_random_walk_std.gyro_bias_std_x, imu.bias_random_walk_std.gyro_bias_std_y, imu.bias_random_walk_std.gyro_bias_std_z);
    PRINT_INFO("IMU Initial Standard Deviation:\n");
    PRINT_INFO("  Position: %e, Velocity: %e, Rotation: %e\n", 
             imu.initial_std.pos_init_std, imu.initial_std.vel_init_std, imu.initial_std.rot_init_std);
    PRINT_INFO("  Gyro Bias: %e, Accel Bias: %e, Gravity: %e\n", 
             imu.initial_std.gyro_bias_init_std, imu.initial_std.acc_bias_init_std, imu.initial_std.gravity_init_std);
